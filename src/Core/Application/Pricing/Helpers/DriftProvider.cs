        ...
        private double CalculateDrift(Underlying underlying, Currency diffusionCurrency, IMarketData marketData, DateTime t_1, DateTime t) {
            ...
            if (underlying is Equity equity) {
                ...
                if (equity.Currency != diffusionCurrency) {
                    ...
                    double sigma_S = marketData.GetUnderlyingMarketData(equity).GetVolatility().GetVolatility(0, DateTime.Today); // TODO: HACK only works with constant vol
                    double sigma_X = marketData.GetUnderlyingMarketData(fxPair).GetVolatility().GetVolatility(0, DateTime.Today);// TODO: HACK only works with constant vol
