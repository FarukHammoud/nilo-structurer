...
namespace Infrastructure {
    public class JsonRateCurveProvider : IRateCurveProvider {
        private readonly string _path;

        public JsonRateCurveProvider(String path) {
            _path = path;
        }

        public async Task<Dictionary<Currency, Curve>> GetRateCurveAsync(IEnumerable<Currency> currencies, CancellationToken ct = default) {
            // placeholder implementation
            return currencies.ToDictionary(
                currency => currency, 
                currency => ExampleSwapCurve);
        }

        private Curve ExampleSwapCurve = new Curve(new OrderedDictionary<DateTime, double>() {
